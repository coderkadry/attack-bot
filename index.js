    ...
    async function initializeFirebaseAdminSDK() {
      try {
        ...
        admin.initializeApp({
          ...
          // Optionally, add databaseURL if you use Realtime Database, not needed for Firestore generally
    ...
    async function getPlayerBalanceFromFirestore(robloxId) {
      ...
      // Use process.env.GOOGLE_CLOUD_PROJECT for the actual Firebase Project ID on the VM
      // Or you can hardcode your project ID if you prefer, but env var is better.
      const projectId = process.env.GOOGLE_CLOUD_PROJECT || 'your-firebase-project-id'; // Replace with your actual project ID or ensure GOOGLE_CLOUD_PROJECT is set
      ...
      try {
        ...
        if (command === 'debug') {
          ...
          if (authReady && db) {
            ...
            debugResults.push(` Firestore path for balances: artifacts/${process.env.GOOGLE_CLOUD_PROJECT || 'your-firebase-project-id'}/public/data/roblox-balances`);
            try {
              // Attempt a small read from Firestore to confirm connectivity
              const testDocRef = db.collection(`artifacts/${process.env.GOOGLE_CLOUD_PROJECT || 'your-firebase-project-id'}/public/data/roblox-balances`).doc('test_read_123');
